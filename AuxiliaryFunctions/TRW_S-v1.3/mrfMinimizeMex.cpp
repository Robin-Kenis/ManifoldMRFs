...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if(oInPtr != NULL){
		...
		if((curField = mxGetField(oInPtr, 0, "maxTime")) != NULL){
			MATLAB_ASSERT(mxGetClassID(curField) == mxDOUBLE_CLASS, "Wrong structure type for options: expected DOUBLE for field <<maxTime>>");
			MATLAB_ASSERT(mxGetNumberOfElements(curField) == 1, "Wrong structure type for options: expected 1 number for field <<maxTime>>");
			options.m_timeMax = (int)(*(double*)mxGetData(curField));
			MATLAB_ASSERT(options.m_timeMax >= 0, "Wrong value for options.maxTime: expected value is >= 0");
		}
